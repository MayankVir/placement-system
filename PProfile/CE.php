...
<html lang="en">
  <head>
    ...
    <title>Placement- CE Students</title>
...
$sql = "SELECT * FROM basicdetails where Approve='1' and Branch='CE' LIMIT $start_from, $num_rec_per_page";
...
$connect = mysqli_connect('localhost','root','','details');
// mysql_select_db('details');
$sql = "SELECT * FROM basicdetails where Approve='1' and Branch='CE'";
$rs_result = $connect->query($sql); //run the query
		...
		if( $currentpage > 1 && $currentpage <= $totalpage)
			{
				...
				$prev = $currentpage-1;
				echo "<li><a  href='CE.php?page=".$prev."'><</a></li>";
...
$prev = $currentpage-1;
	for ($i=$prev+1; $i<=$currentpage+2; $i++){
		echo "<li><a href='CE.php?page=".$i."'>".$i."</a></li>";
  }
  }
	...
	{
		$nxt = $currentpage+1;
		echo "<li><a  href='CE.php?page=".$nxt."' >></a></li>";
	}
